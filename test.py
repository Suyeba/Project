...
def predict_major_incident(day_of_week, location, attack_type, perpetrator, victims_injured, victims_deceased,
    ...
    input_data[category_mapping['attack_type'].index(attack_type)] = 1
    input_data[len(category_mapping['attack_type']) + category_mapping['perpetrator'].index(perpetrator)] = 1
    input_data[len(category_mapping['attack_type']) + len(category_mapping['perpetrator']) + category_mapping[
        'target_type'].index(target_type)] = 1
    input_data[len(category_mapping['attack_type']) + len(category_mapping['perpetrator']) + len(
        category_mapping['target_type']) + category_mapping['weapon_used'].index(weapon_used)] = 1
    input_data[len(category_mapping['attack_type']) + len(category_mapping['perpetrator']) + len(
